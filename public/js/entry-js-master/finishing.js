...
function gagal() {
    ...
    $('#finishingID').parent('div').append('<span class="help-block form-error" id="spanErrorID">Finishing ID has already been taken</span>');
    $('#finishingID').parent('div').removeClass('has-success');
    $('#finishingID').parent('div').addClass('has-error');
    $('#finishingID').css("border-color", "rgb(169, 68, 66)");
}
function sukses() {
    $('#spanErrorID').remove();
    $('#finishingID').parent('div').removeClass('has-error');
    $('#finishingID').parent('div').addClass('has-success');
    $('#finishingID').css("border-color", "");
}
function validationID(dataID, status, r) {
    dataID = decryptDataID(dataID, r);
    var search = $('#finishingID').val();
    if (search == '') {
        return false;
    } else if (searchID(search, dataID, 'finishingID')) {
        gagal();
        if (status == 1) {
...
$(".btn-edit").click(function () {
    ...
    $('#finishingNameUpdate').val(searchData("FinishingName", data));

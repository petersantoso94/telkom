...
$(document).ready(function () {
    $("#searchPurchaseReturn").keydown(function (event) {
        if (event.keyCode == 13) { //enter
            event.preventDefault();
            $.post(getResultSearchPR, {id: $("#searchPurchaseReturn").val()}).done(function (data) {
                $("#selectPurchaseReturn").html(data);
            });
        }
    });

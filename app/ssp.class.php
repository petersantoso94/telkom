        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
                ...
                if (isset($column['formatter'])) {
                    $tampColumnDb = $column['db'];
                    $pos = strpos($tampColumnDb, 'InternalID');
                    if ($pos) {
                        $tampColumnDb = 'InternalID';
                    }
                    $row[$column['dt']] = $column['formatter']($data[$i][$tampColumnDb], $data[$i]);
                } else {
...
//                    $binding = self::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
        ...
        if (isset($request['columns'])) {
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true' &&
                        $str != '') {
                    $binding = self::bind($bindings, '%' . str_replace(" ", "%", $str) . '%', PDO::PARAM_STR);
...
//            $inventory = DB::select(DB::raw("SELECT * FROM " . $table . " " . $join . " " . $where . " " . $extraWhere . " " . $order . " " . $limit . " "))->get();
//            foreach ($inventory as $value) {
//                $inventory = Inventory::find($value[0]);
//                $inventory->TampLastPrice = 'Rp. ' . getLastPriceThisInventory($inventory->InternalID);
//                $inventory->TampStock = getEndStockInventory($inventory->InternalID);
//                $inventory->TampHPP = 'Rp. ' . getHPPValueInventory($inventory->InternalID);
//                $inventory->save();
//            }
        }

...
$(document).ready(function () {
    ...
    $("#searchPurchaseOrder").keydown(function (event) {
        if (event.keyCode == 13) { //enter
            event.preventDefault();
            $.post(getResultSearchPO, {id: $("#searchPurchaseOrder").val()}).done(function (data) {
                $("#selectPurchaseOrder").html(data);
            });
        }
    });

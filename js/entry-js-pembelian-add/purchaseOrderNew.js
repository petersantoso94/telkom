$(document).ready(function () {
    ...
    function hitungTotal() {
        ...
        if ($("#paymentCredit").attr('checked') == 'checked') {
            ...
            $.post(checkRecieveablePO, {total: tampTotal, coa6: $("#coa6").val()}).done(function (data) {
                var tamp = data.split('---;---');
                if (tamp[0] == "1") {
     ...
     * 
     ...
     //get price uom
     $.post(getPriceRangeThisInventoryPO, {InventoryID: $("#inventory-0").val(), UomID: $("#uom-0").val() , Qty: $("#price-" + + id[1]+ "-qty").val(), urutan: 0}).done(function (data) {
    ...
    $("#btn-addRow").click(function () {
        ...
        $('#table-purchaseOrder tr:last').after('<tr id="row' + baris + '">' +
                ...
                '</td>' +
                '<td class="text-right">' +
                '<input type="text" class="maxWidth discount right input-theme numajaDesimal" name="discount1[]" min="0" max="100" id="price-' + baris + '-discount1" value="0.00">' +
                '</td>' +
                '<td class="text-right">' +
        ...
        $('#price-' + baris + '-discount1').val($('#price-0-discount1').val());
        ...
        //get price uom
//        $.post(getPriceRangeThisInventoryPO, {InventoryID: $("#inventory-" + baris).val(), UomID: $("#uom-" + baris).val(), Qty: $("#price-" + + id[1]+ "-qty").val(), urutan: baris}).done(function (data) {
//            var arrData = data.split("---;---");
//            if (arrData[0] != 0) {
//                $("#price-" + arrData[1]).val(addPeriod(arrData[0].trim(), ","));
//                changeTotal($("#price-" + arrData[1]).attr("id"));
...
//        });
    ...
    $("#date").change(function () {
        var cariText = '';
        $('#purchaseOrderID').load(a, {"date": $("#date").val()})
    });
    $(function () {
        $('#vat').change(function () {

$(document).ready(function () {
    ...
    $(".inventory").change(function () {
        ...
        if (tampVar.indexOf("inventory") >= 0) {
            ...
            $.post(getUomThisInventory, {id: $('#inventory-' + id[1]).val()}).done(function (data2) {
                $("#uom-" + id[1]).html(data2);
                //get price uom
                $.post(getPriceRangeThisInventoryQuotation, {InventoryID: $("#inventory-" + id[1]).val(), UomID: $("#uom-" + id[1]).val(), Qty: $("#price-" + id[1] + '-qty').val(), urutan: id[1]}).done(function (data) {
    ...
    $("#btn-addRow").click(function () {
        ...
        $('#table-quotation tr:last').after('<tr id="row' + baris + '">' +
                '<td class="chosen-uom">' +
        ...
        //get price uom
//        $.post(getPriceRangeThisInventoryQuotation, {InventoryID: $("#inventory-" + baris).val(), UomID: $("#uom-" + baris).val(), Qty: $("#price-" + id[1] + '-qty').val(), urutan: baris}).done(function (data) {
//            var arrData = data.split("---;---");
//            if (arrData[0] != 0) {
//                $("#price-" + arrData[1]).val(addPeriod(arrData[0].trim(), ","));
//                changeTotal($("#price-" + arrData[1]).attr("id"));
...
//        });
    ...
    $("#date").change(function () {
        var cariText = '';
        $('#quotationID').load(cariS, {"date": $("#date").val()})
    });
    $(function () {
        $('#vat').change(function () {

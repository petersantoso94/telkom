...
$("#btn-addRow").click(function () {
    $('#table-purchase tr:last').after('<tr id="row' + baris + '">' + '<td class="chosen-transaction">' + textSelect + '</td>' + '<td class="text-right">' + '<input type="text" class="maxWidth qty right input-theme" name="qty[]" maxlength="11" min="1" value="1" id="price-' + baris + '-qty">' + '</td>' + '<td class="text-right">' + '<input type="text" class="maxWidth price right numajaDesimal input-theme" name="price[]" maxlength="" value="0.00" id="price-' + baris + '">' + '</td>' + '<td class="text-right">' + '<input type="text" class="maxWidth discount numajaDesimal right input-theme" name="discount[]" min="0" max="100" id="price-' + baris + '-discount" value="0.00">' + '</td>' + '<td class="text-right">' + '<input type="text" class="maxWidth discountNominal numajaDesimal right input-theme" name="discountNominal[]" id="price-' + baris + '-discountNominal" value="0.00">' + '</td>' + '<td class="right subtotal" id="price-' + baris + '-qty-hitung">' + '0.00' + '</td>' + '<td>' + '<button class="btn btn-pure-xs btn-xs btn-deleteRow" type="button" data="row' + baris + '"><span class="glyphicon glyphicon-trash"></span></button>' + '</td>' + '</tr>');
...
function autoChangeTotal() {
    myFunctionduit();
    function addPeriodPurchase(nStr, add) {
        nStr += '';
        x = nStr.split(add);
        x1 = x[0];
    ...
    function changeTotal(id) {
        ...
        output = Math.round(output * 100) / 100;
        output = addPeriodPurchase(output, ',');
        if (output.indexOf(".") == -1) {
            output = output + '.00';
...
function hitungTotal() {
    ...
    document.getElementById("total").innerHTML = addPeriodPurchase(total.toFixed(2), ',');
    document.getElementById("grandTotal").innerHTML = (addPeriodPurchase((total - diskon - dp).toFixed(2), ','));
    var tamp = total - diskon - dp;
    if (document.getElementById("vat").checked == true) {
        var tax = tamp / 10;
        tax = Math.round(tax * 100) / 100;
        document.getElementById("tax").innerHTML = addPeriodPurchase(tax.toFixed(2), ',');
        var grandTotal = tamp + tax;
        grandTotal = Math.round(grandTotal * 100) / 100;
        document.getElementById("grandTotalAfterTax").innerHTML = addPeriodPurchase(grandTotal.toFixed(2), ',');
...
$(document).ready(function () {
    ...
    $("#rate").val(addPeriodPurchase($("#rate").val(), ','));
